...
def linear_schedule(step: int, val_start: float, val_end: float, period: int, start_step: int=0) -> float:
    """
    ...
    if step >= start_step + period:
        ...
        return val_start + (val_end - val_start) * (step - start_step) / period
...
def sample_patch_params(batch_size: int, patch_cfg: "EasyDict", device: str='cpu') -> Dict:
    """
...
def run_batchwise(fn: Callable, data: Dict[str, torch.Tensor], batch_size: int, dim: int=0, **kwargs) -> Any:
    ...
    assert len(set([v.shape[dim] for v in values])) == 1, \
        f"Tensors must be of the same size along dimension {dim}. Got {dict(zip(keys, [v.shape[dim] for v in values]))}"
...
def sample_random_c(batch_size, c_dim, device):
    if c_dim > 0:
        c_idx = torch.randint(low=0, high=c_dim, device=device, size=(batch_size,)) # [num_samples]
        c = torch.zeros(batch_size, c_dim, device=device) # [num_samples, c_dim]
        c[torch.arange(batch_size), c_idx] = 1.0
    else:
        c = torch.empty(batch_size, c_dim, device=device) # [num_samples, c_dim]
    return c
